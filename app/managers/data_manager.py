...
class DataManager:
    ...
    def __init__(
        ...
        self.data_semaphore = asyncio.Semaphore(
            100
        )  # Increased from 20 to 100 for better throughput

    async def get_asset_data(self, symbol: str) -> AssetData | None:
        """
        ...
        try:
            ...
            # Get instrument info
            instrument_info = FINANCIAL_INSTRUMENTS.get(symbol, {"name": symbol, "type": "asset"})
    ...
    def _generate_mock_asset_data(self, symbol: str, instrument_info: dict) -> AssetData:
        ...
        return {
            ...
            "market_cap": (
                random.randint(1000000000, 1000000000000)
                if instrument_info["type"] == "stock"
                else None
            ),
