...
class AlertService:
    ...
    async def create_price_alert(
        ...
        try:
            ...
            self.alert_states[alert_id] = {"last_checked_price": None, "triggered_at": None}
    ...
    async def _get_current_price(self, symbol: str, data_fetcher) -> float | None:
    ...
    async def send_email_notification(self, user: User, subject: str, message: str):
        ...
        try:
            ...
            text = msg.as_string()
            server.sendmail(
                self.sender_email or self.email_username, str(getattr(user, "email", "")), text
            )
            server.quit()
