...
class AdvancedAlertService:
    ...
    async def create_alert(
        ...
        try:
            ...
            db_alert = Alert(
                ...
                extra_params=(
                    json.dumps(condition.get("extra_params", {}))
                    if condition.get("extra_params")
                    else None
                ),
    ...
    async def update_alert(self, alert_id: int, **kwargs) -> Any | None:
            ...
            try:
                ...
                for key, value in kwargs.items():
                    if hasattr(db_alert, key):
                        if (
                            key in ["notification_types", "schedule", "extra_params"]
    ...
    async def _refresh_active_alerts(self):
        ...
        try:
            ...
            db = SessionLocal()
            try:
                db_alerts = db.query(Alert).filter(Alert.is_active).all()
    ...
    async def _is_alert_active_by_schedule(self, alert) -> bool:
        ...
        try:
            ...
            if schedule_data.get("active_days") and current_day not in schedule_data.get(
                "active_days", []
            ):
                return False
    ...
    async def _check_alert_condition(self, alert):
        ...
        try:
            ...
            condition_met, current_value = await self._evaluate_condition(
                condition_data, current_data, alert.symbol
            )
    ...
    async def _get_asset_data(self, symbol: str) -> dict | None:
    ...
    async def _evaluate_condition(
        self, condition: dict, current_data: dict, symbol: str
    ) -> tuple[bool, float]:
            ...
            if condition_type == "price_above":
                ...
                try:
                    ...
                    if not df.empty:
                        ...
                        return (macd > signal) == condition.get("extra_params", {}).get(
                ...
                if user:
                    for notification_type in notification_types:
                        await self._send_notification(
                            notification_type, user, alert, triggered_value, condition_met
                        )
    ...
    async def _send_notification(
        self, notification_type: str, user, alert, triggered_value: float, condition_met: dict
    ):
        ...
        try:
            ...
            if notification_type == "email" and user.email:
                await self._send_email_notification(
                    user.email, f"Alert for {alert.symbol}", message
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "monitoring_task_active": self.monitoring_task is not None
