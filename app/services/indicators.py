...
class TechnicalIndicators:
    ...
    def calculate_macd(prices: pd.Series) -> dict[str, float]:
        ...
        try:
            ...
            signal_val = (
                float(signal.iloc[-1])
                if not signal.empty and not np.isnan(signal.iloc[-1])
                else 0.0
            )
            histogram_val = (
                float(histogram.iloc[-1])
                if not histogram.empty and not np.isnan(histogram.iloc[-1])
                else 0.0
            )

            return {"macd": macd_val, "signal": signal_val, "histogram": histogram_val}
        except Exception as e:
    ...
    def calculate_bollinger_bands(
        ...
        try:
            ...
            upper_val = (
                ...
                if not upper_band.empty and not np.isnan(upper_band.iloc[-1])
                else 0.0
            )
            middle_val = (
                float(sma.iloc[-1]) if not sma.empty and not np.isnan(sma.iloc[-1]) else 0.0
            )
            lower_val = (
                float(lower_band.iloc[-1])
                if not lower_band.empty and not np.isnan(lower_band.iloc[-1])
                else 0.0
            )

            return {"upper": upper_val, "middle": middle_val, "lower": lower_val}
        except Exception as e:
    ...
    def calculate_ichimoku_cloud(
        ...
        try:
            ...
            tenkan_val = (
                float(tenkan_sen.iloc[-1])
                if not tenkan_sen.empty and not np.isnan(tenkan_sen.iloc[-1])
                else 0.0
            )
            kijun_val = (
                float(kijun_sen.iloc[-1])
                if not kijun_sen.empty and not np.isnan(kijun_sen.iloc[-1])
                else 0.0
            )
            senkou_a_val = (
                float(senkou_span_a.iloc[-1])
                if not senkou_span_a.empty and not np.isnan(senkou_span_a.iloc[-1])
                else 0.0
            )
            senkou_b_val = (
                float(senkou_span_b.iloc[-1])
                if not senkou_span_b.empty and not np.isnan(senkou_span_b.iloc[-1])
                else 0.0
            )
            chikou_val = (
                float(chikou_span.iloc[0])
                if not chikou_span.empty and not np.isnan(chikou_span.iloc[0])
                else 0.0
            )
    ...
    def calculate_fibonacci_retracement(high: float, low: float) -> dict[str, float]:
        ...
        try:
            if high <= low:
                return {"23.6%": low, "38.2%": low, "50.0%": low, "61.8%": low, "78.6%": low}
            ...
            return {"23.6%": 0.0, "38.2%": 0.0, "50.0%": 0.0, "61.8%": 0.0, "78.6%": 0.0}
    ...
    def calculate_williams_r(
        ...
        try:
            ...
            return (
                ...
                if not williams_r.empty and not np.isnan(williams_r.iloc[-1])
                else 0.0
            )
        except Exception as e:
    ...
    def calculate_parabolic_sar(
        ...
        try:
            ...
            af_val = (
                float(af.iloc[-1]) if not af.empty and not np.isnan(af.iloc[-1]) else acceleration
            )
    ...
    def calculate_momentum(prices: pd.Series, period: int = 10) -> float:
        ...
        try:
            ...
            return (
                ...
                if not momentum.empty and not np.isnan(momentum.iloc[-1])
                else 0.0
            )
        except Exception as e:
    ...
    @staticmethod
    def calculate_all_indicators(df: pd.DataFrame) -> dict[str, float | dict]:
        ...
        try:
            ...
            indicators = {
                ...
                "ichimoku": TechnicalIndicators.calculate_ichimoku_cloud(
                    high_prices, low_prices, close_prices
                ),
                ...
                "parabolic_sar": TechnicalIndicators.calculate_parabolic_sar(
                    high_prices, low_prices, close_prices
                ),
            ...
            if len(high_prices) > 0 and len(low_prices) > 0:
                indicators["fibonacci"] = TechnicalIndicators.calculate_fibonacci_retracement(
                    float(high_prices.max()), float(low_prices.min())
                )
