...
class PortfolioService:
    ...
    async def get_user_portfolios(self, user_id: int) -> list[dict]:
        ...
        try:
            ...
            for p in portfolios:
                ...
                created_at_str = created_at.isoformat() if created_at is not None else None
                result.append({"id": p.id, "name": str(p.name), "created_at": created_at_str})
            return result
        except Exception as e:
    ...
    async def _get_current_price(self, symbol: str, asset_type: str) -> float | None:
    ...
    async def _calculate_sharpe_ratio(
        self, holdings_data: list[dict], risk_free_rate: float = 0.02
    ) -> float | None:
    ...
    async def _calculate_max_drawdown(self, holdings_data: list[dict]) -> float | None:
    ...
    async def _calculate_volatility(self, holdings_data: list[dict]) -> float | None:
        ...
        try:
            ...
            if volatilities and weights:
                weighted_volatility = sum(
                    w * v for w, v in zip(weights, volatilities, strict=False)
                )
    ...
    async def calculate_value_at_risk(
        self, portfolio_id: int, confidence_level: float = 0.95, time_horizon: int = 1
    ) -> dict:
    ...
    async def calculate_portfolio_beta(
        ...
        try:
            ...
            for _i in range(30):  # Last 30 days
            ...
            benchmark_variance = sum(
                (benchmark_returns[i] - benchmark_mean) ** 2 for i in range(len(benchmark_returns))
    ...
    async def calculate_sortino_ratio(
        self, portfolio_id: int, risk_free_rate: float = 0.02
    ) -> dict:
        ...
        try:
            ...
            if len(negative_returns) > 0:
                downside_deviation = (
                    sum((r - 0) ** 2 for r in negative_returns) / len(negative_returns)
                ) ** 0.5
            else:
            ...
            ) / downside_deviation  # Daily risk-free rate
    ...
    async def get_portfolio_history(self, portfolio_id: int, days: int = 30) -> dict:
        ...
        try:
            items = self.db_service.get_portfolio_items(portfolio_id)
            if not items:
                return {"portfolio_id": portfolio_id, "days": days, "history": []}
            ...
            for i in range(days):
                ...
                history.append({"date": date.isoformat(), "value": round(portfolio_value, 2)})

            return {"portfolio_id": portfolio_id, "days": days, "history": history}
        except Exception as e:
