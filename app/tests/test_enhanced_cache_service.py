...
class TestEnhancedCacheService:
    ...
    def test_partitioning(self):
        ...
        async def test_async():
            ...
            stock_result = await self.cache_service.set(
                "stock_AAPL_data", {"type": "stock", "symbol": "AAPL"}
            )
            crypto_result = await self.cache_service.set(
                "crypto_bitcoin_data", {"type": "crypto", "symbol": "bitcoin"}
            )
            forex_result = await self.cache_service.set(
                "forex_EURUSD_data", {"type": "forex", "symbol": "EURUSD"}
            )
            general_result = await self.cache_service.set(
                "general_data", {"type": "general", "value": "test"}
            )
