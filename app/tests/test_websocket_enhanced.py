...
async def test_websocket_manager_data_stream_worker():
    ...
    with patch.object(
        manager.subscription_manager, "get_all_subscribed_symbols", return_value=["AAPL"]
    ):
        # Mock the data manager's get_assets_data method
        with patch.object(
            manager.data_manager, "get_assets_data", return_value=[{"symbol": "AAPL"}]
        ):
            # Mock the subscription manager's get_symbol_subscribers method
            with patch.object(
                manager.subscription_manager,
                "get_symbol_subscribers",
                return_value=["test_client_id"],
            ):
                # Mock the connection manager's active_connections
                mock_websocket = AsyncMock()
                manager.connection_manager.active_connections[mock_websocket] = {
                    "id": "test_client_id"
                }
